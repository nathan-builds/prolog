...
hitting_integer_powers(X,Y,T,Pa,Pb):-
int_powers_helper(X,Y,T,Pa,Pb,1,1).
    

int_powers_helper(X,Y,T,Ea,Eb,Ea,Eb):-
Ynew is Y**Eb,
Xnew is X**Ea,
Z is min(Xnew,Ynew),
W is abs(Xnew - Ynew),
...
int_powers_helper(X,Y,T,Pa,Pb,Ea,Eb):-
Ynew is Y**Eb,
Xnew is X**Ea,
Ynew is min(Xnew,Ynew),
Eb1 is Eb + 1,
int_powers_helper(X,Y,T,Pa,Pb,Ea, Eb1).

int_powers_helper(X,Y,T,Pa,Pb,Ea,Eb):-
Ynew is Y**Eb,
Xnew is X**Ea,
Xnew is min(Xnew,Ynew),
Ea1 is Ea + 1,
int_powers_helper(X,Y,T,Pa,Pb,Ea1, Eb).
...
josephus(L,I,Last):-
    IndexNew is I-1,
    josephus(L,IndexNew,Last,0).


josephus([L],_,L,_):-
    !.

josephus(L,I,Last,Start):-
    length(L,Len),
    IndexNew is I+Start,
    IndexNew2 is (IndexNew mod Len),
    nth0(IndexNew2,L,_,Remainder),
    josephus(Remainder,I,Last,IndexNew2). 


extract_increasing(Digits, Nums):-
    string_chars(Digits, DigitsAsList), 
    [FirstItem, SecondItem| T] = DigitsAsList, 
    atom_number(FirstItem, NewFirst),
    atom_number(SecondItem, NewSecond),
        extract_helper(NewFirst, NewSecond, T, [NewFirst], Nums).

extract_helper(F, S, [],Storage, Nums):-
    ( F< S ->  
    append([S], Storage, NewStorage), 
    reverse(NewStorage, Nums) ; reverse(Storage, Nums) ),
    !.

extract_helper(F, S, DigitAsList, Storage, Nums):-
    F < S,
    append([S], Storage, NewStorage),
    [N | T] = DigitAsList,
    atom_number(N, NextInt),
    extract_helper(S, NextInt, T, NewStorage, Nums), !.
    

extract_helper(F, S, DigitAsList, Storage, Nums):-
    [N | T] = DigitAsList,
        string_concat(S, N, NewNext),
    atom_number(NewNext, NextInt),
    extract_helper(F, NextInt, T, Storage, Nums).
